...
extern "C" DLLEXPORT void pluginProcess(const std::shared_ptr<Doppelganger::Room> &room, const nlohmann::json &parameters, nlohmann::json &response, nlohmann::json &broadcast)
{
	...
	//  ]
	// }
	// <when core.config.at("output").at("type") == "storage">
	// response = {
	// }
	// broadcast = {
	// }
	...
	std::string base64Str;
	{
		...
		if (room->customData.find("saveScreenshotAll") == room->customData.end())
		{
			room->customData["saveScreenshotAll"] = std::unordered_map<std::string, std::pair<std::vector<bool>, std::string>>();
		}
		std::unordered_map<std::string, std::pair<std::vector<bool>, std::string>> &packetsVec = boost::any_cast<std::unordered_map<std::string, std::pair<std::vector<bool>, std::string>> &>(room->customData.at("saveScreenshotAll"));
